...
public class AlunoDAO {
    ...
    public void incluir(Aluno aluno) throws SQLException, ClienteException {
        if (aluno == null)
            ...
            this.updateQuery("INSERT INTO " + "aluno (nome, cpf, telefone, email, matricula) VALUES (" + "\"" + aluno.getNome()
                    + "\", " + "\"" + aluno.getCpf() + "\", " + "\"" + aluno.getTelefone() + "\", " + "\"" + aluno.getEmail()
                    + "\", " + "\"" + aluno.getMatricula() + "\"); ");
        } else
    ...
    public void alterar(Aluno aluno_velho, Aluno aluno_novo) throws SQLException, ClienteException {
        ...
        if (!this.inDB(aluno_velho))
            ...
            String msg = "UPDATE aluno SET " + "nome = \"" + aluno_novo.getNome() + "\", " + "cpf = \"" + aluno_novo.getCpf()
                    + "\", " + "telefone = \"" + aluno_novo.getTelefone() + "\", " + "email = \"" + aluno_novo.getEmail() + "\", "
                    + "matricula = \"" + aluno_novo.getMatricula() + "\"" + " WHERE " + "aluno.nome = \"" + aluno_velho.getNome()
                    + "\" and " + "aluno.cpf = \"" + aluno_velho.getCpf() + "\" and " + "aluno.telefone = \""
                    + aluno_velho.getTelefone() + "\" and " + "aluno.email = \"" + aluno_velho.getEmail() + "\" and "
    ...
    public void excluir(Aluno aluno) throws SQLException, ClienteException {
        if (aluno == null)
            ...
            this.updateQuery("DELETE FROM aluno WHERE " + "aluno.nome = \"" + aluno.getNome() + "\" and " + "aluno.cpf = \""
    ...
    private boolean inDB(Aluno aluno) throws SQLException {
        return this.inDBGeneric("SELECT * FROM aluno WHERE " + "aluno.nome = \"" + aluno.getNome() + "\" and " + "aluno.cpf = \""
                + aluno.getCpf() + "\" and " + "aluno.telefone = \"" + aluno.getTelefone() + "\" and " + "aluno.email = \""
                + aluno.getEmail() + "\" and " + "aluno.matricula = \"" + aluno.getMatricula() + "\";");
    }

    private boolean inDBCpf(String codigo) throws SQLException {
        return this.inDBGeneric("SELECT * FROM aluno WHERE " + "aluno.cpf = \"" + codigo + "\";");
    }

    private boolean inDBMatricula(String codigo) throws SQLException {
        return this.inDBGeneric("SELECT * FROM aluno WHERE " + "aluno.matricula = \"" + codigo + "\";");
    }

    private boolean inOtherDB(Aluno aluno) throws SQLException, ClienteException {
        return this.inDBGeneric("SELECT * FROM reserva_sala_aluno WHERE " + "id_aluno = (SELECT id_aluno FROM aluno WHERE "
                + "aluno.nome = \"" + aluno.getNome() + "\" and " + "aluno.cpf = \"" + aluno.getCpf() + "\" and "
                + "aluno.telefone = \"" + aluno.getTelefone() + "\" and " + "aluno.email = \"" + aluno.getEmail() + "\" and "
                + "aluno.matricula = \"" + aluno.getMatricula() + "\");");
    }

    private Aluno fetchAluno(ResultSet rs) throws ClienteException, SQLException {
        return new Aluno(rs.getString("nome"), rs.getString("cpf"), rs.getString("matricula"), rs.getString("telefone"),
                rs.getString("email"));
    }
